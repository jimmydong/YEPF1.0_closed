...
class Cache implements CacheInterface
{
    	...
    	if('memcached' == $type)
    	{
			...
			if(!empty($serverList))
			{
				foreach($serverList as $v)
				{
			           ...
			           if($is_sucess === false)
			                         \Debug::log('memcacheConnectError'. $is_sucess,'ip:'.$v['host'].":".$v['port'],__FILE__.':'.__LINE__);  
    ...
    public function set($cacheKey, $cacheValue, $lifetime = 0)
    {
    	...
    	$cacheKey = $this->getKey($cacheKey);
    	if(empty($cacheKey)) return false;
    	if(method_exists($this->cache, 'setOption')) $re = $this->cache->set($cacheKey, $cacheValue, $lifetime);
